...
def purchase_video(
    ...
    try:
        ...
        try:
            success, block_hash, err_msg = transfer_with_timeout(
...
def revert_pending_purchase(video_id, miner_hotkey, miner_hotkey_signature):
    ...
    if revert_response.status_code != 200:
        print(
            f"{RED}Error reverting pending purchase of video {video_id}: {revert_response.status_code}{RESET}"
        )
        return
    if revert_response.status_code == 200:
        print(
            f"{GREEN}Pending purchase of video {video_id} reverted successfully.{RESET}"
        )
    return
