...
def get_relevant_timestamps(
    query: str, yt: video_utils.YoutubeDL, video_path: str, max_length: int
) -> Tuple[int, int]:
    """
...
def search_and_embed_youtube_videos(
    query: str, num_videos: int, imagebind: ImageBind
) -> List[VideoMetadata]:
    """
        ...
        for result in results:
            ...
            download_path = video_utils.download_youtube_video(
                ...
                start=0,
                    ...
                    result.length, FIVE_MINUTES
                ),  # download the first 5 minutes at most
            )
            if download_path:
                clip_path = None
                try:
                    result.length = video_utils.get_video_duration(
                        download_path.name
                    )  # correct the length
                    bt.logging.info(
                        f"Downloaded video {result.video_id} ({min(result.length, FIVE_MINUTES)}) in {time.time() - start} seconds"
                    )
                    start, end = get_relevant_timestamps(
                        query, result, download_path, max_length=MAX_VIDEO_LENGTH
                    )
...
def search_and_diarize_youtube_videos(
    query: str,
    num_videos: int,
    diarization_pipeline: CustomDiarizationPipeline,
    imagebind: ImageBind,
) -> List[AudioMetadata]:
    """
        ...
        for result in results:
            ...
            download_path = video_utils.download_youtube_video(
                ...
                start=0,
                    ...
                    result.length, MAX_AUDIO_LENGTH_SECONDS
                ),  # download the first 5 minutes at most
            )
            if download_path:
                clip_path = None
                try:
                    ...
                    bt.logging.info(
                        f"Downloaded audio {result.video_id} ({min(result.length, MAX_AUDIO_LENGTH_SECONDS)}) in {time.time() - start_time_loop} seconds"
                    )
                    start, end = get_relevant_timestamps(
                        query,
                        result,
                        download_path,
                        max_length=MAX_AUDIO_LENGTH_SECONDS,
                    )
                    ...
                    embeddings = imagebind.embed([description], [clip_path])
                    bt.logging.info(
                        f"Embeddings: {type(embeddings)}, audio_emb: {type(embeddings.audio[0])}, audio_array: {type(audio_array)} {audio_array.shape}, audio_bytes: {type(audio_bytes)}, sr: {sr}, diar_timestamps_start: {type(diar_timestamps_start)}, diar_timestamps_end: {type(diar_timestamps_end)}, diar_speakers: {type(diar_speakers)}"
                    )
                    bt.logging.info(
                        f"Audio duration: {end - start}, actual length: {result.length}"
                    )
            ...
            bt.logging.info(
                f"Audio Time taken for loop: {end_time_loop - start_time_loop}"
            )
