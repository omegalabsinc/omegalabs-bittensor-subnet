...
class AudioScore:
    ...
    def speech_content_score(self, audio_arr, sr):
        ...
        output = self.pipeline(
            {
                "waveform": torch.from_numpy(audio_arr.astype(np.float32))
                .unsqueeze(0)
                .to(self.device),
                "sample_rate": sr,
            }
    ...
    def speaker_dominance_score(
        self, timestamps_start, timestamps_end, speakers, dominance_threshold=0.7
    ):
        if timestamps_start is None:
            self.rttm_data = None
            return 0
        self.rttm_data = pd.DataFrame(
            {"start": timestamps_start, "end": timestamps_end, "speaker": speakers}
        )
    ...
    def background_noise_score(self, audio_arr, sr, noise_threshold=0.1):
        ...
        frames = librosa.util.frame(
            self.audio, frame_length=frame_length, hop_length=frame_length
        )
        ...
        return 1 - max(
            0, 1 - (snr / 50)
        )  # Normalize to 0-1 range, assuming 50dB as reference
    ...
    def total_score(self, audio_arr, sr, timestamps_start, timestamps_end, speakers):
        ...
        speaker_dominance_score = self.speaker_dominance_score(
...
if __name__ == "__main__":
    ...
    ds = load_dataset(
        repo_id, split="test", cache_dir="/workspace/tezuesh/voxconverse/data_cache"
    )
    ...
    score = audio_score.total_score(
        audio_arr, sr, timestamps_start, timestamps_end, speakers
    )
    print(score)
