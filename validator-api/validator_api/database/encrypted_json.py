...
class EncryptedJSON(TypeDecorator):
    ...
    def process_bind_param(self, value: Optional[JSONType], dialect: Dialect) -> Optional[bytes]:
    ...
    def process_result_value(self, value: Optional[bytes], dialect: Dialect) -> Optional[JSONType]:
        if value is not None:
            try:
...
class LargeEncryptedJSON(EncryptedJSON):
    impl = LargeBinary(length=4 * 1024 * 1024 * 1024 - 1)  # 4 GB - 1 byte because thats the MySQL max

class MediumEncryptedJSON(EncryptedJSON):
    impl = LargeBinary(length=16 * 1024 * 1024 - 1)  # 16 MB - 1 byte (MySQL MEDIUMBLOB max size)
...
def test_encrypted_json():
    ...
    test_cases = [
        ...
        True,  # bool
        None,  # null
        {"nested": {"list": [1, 2, 3], "dict": {"a": 1, "b": 2}}},  # complex nested structure
        FakeModel(name="Test", value=123),  # Pydantic BaseModel
        NestedFakeModel(nested=FakeModel(name="Nested", value=456)),  # Nested Pydantic BaseModel
    ]
