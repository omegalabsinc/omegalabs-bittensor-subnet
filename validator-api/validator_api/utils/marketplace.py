...
from validator_api.config import NETWORK, BT_TESTNET, NETUID, FOCUS_REWARDS_PERCENT, MAX_FOCUS_POINTS, MAX_FOCUS_POINTS_PER_HOUR
...
async def get_dollars_available_today() -> float:
    ...
    return max_focus_tao * tao_price
...
async def estimate_tao(productive_score: float, video_duration: float):
    try:
        ...
        tao = actual_focus_points_for_video / max_focus_points_available_today * float(max_focus_tao)
        return round(tao, 5)
